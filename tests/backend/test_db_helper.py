from backend import db_helper

def test_fetch_expenses_for_date_aug_15():
    expenses = db_helper.fetch_expenses_for_date("2024-08-15")

    assert len(expenses) == 1
    assert expenses[0]['amount'] == 10.0
    assert expenses[0]['category'] == "Shopping"
    assert expenses[0]['notes'] == "Bought potatoes"  # Corrected field to 'notes'

def test_fetch_expenses_for_invalid_date():
    expenses = db_helper.fetch_expenses_for_date("2999-08-15")
    assert len(expenses) == 0

def test_fetch_expenses_for_invalid_range():
    summery = db_helper.fetch_expenses_summery("2088-01-01", "2088-12-01")
    assert len(summery) == 0
